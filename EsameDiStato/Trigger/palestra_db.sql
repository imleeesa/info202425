...
--
-- Host: 127.0.0.1
-- Creato il: Feb 11, 2025 alle 09:27
...
(1, 15, 19, 18, 'Mercoledi', 1),
(2, 20, 14, 19, 'Mercoledi', 2),
(3, 25, 10, 17, 'Lunedi', 3),
(4, 30, 12, 20, 'Venerdi\r\n', 4),
...
--
DELIMITER $$
CREATE TRIGGER `after_reservation_delete` AFTER DELETE ON `prenotazioni` FOR EACH ROW BEGIN
    UPDATE offerta
    SET capienza = capienza + 1
    WHERE id = OLD.idOfferta;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `after_reservation_insert` AFTER INSERT ON `prenotazioni` FOR EACH ROW BEGIN
    UPDATE offerta
    SET capienza = capienza - 1
    WHERE id = NEW.idOfferta AND capienza > 0;
END
$$
DELIMITER ;
